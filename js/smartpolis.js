function smartpolis() {
	...
	self.updateCarMarks = function() {
		...
		jQuery.getJSON('/wp-content/plugins/smartpolis/php/ajax.php', {'type':'car_marks'}, function(r) {
			if ( r.length > 0 ) {
	...
	self.updateCarModels = function() {
		...
		jQuery.getJSON('/wp-content/plugins/smartpolis/php/ajax.php', {'type':'car_models', 'car_mark':jQuery('#smartpolis_car_marks').val()}, function(r) {
			if ( r.length > 0 ) {
				jQuery(r).each(function() {
	...
	self.updateCarModifications = function() {
		...
		jQuery.getJSON('/wp-content/plugins/smartpolis/php/ajax.php', {'type':'car_modifications', 'car_model':jQuery('#smartpolis_car_models').val()}, function(r) {
			if ( r.length > 0 ) {
				jQuery(r).each(function() {
	...
	self.updateDriversCount = function() {
		...
		if (val == 'multiply') {
			...
			for(i=1; i<=val; i++) {
				h+='<div class="row">';
				h+='<div class="bl">';
				h+='<label>Возраст</label><select class="w110" name="car_driver_age[]">';
				for(z=18; z<=60; z++) {
				...
				h+='</div><!--end bl-->';
				h+='<div class="bl">';
				h+='<label>Стаж</label><select class="w110" name="car_driver_prof[]">';
				...
				h+='</div><!--end bl-->';
				h+='<div class="bl">';
				h+='<label>Пол</label><select class="w110" name="car_driver_gender[]">';
	...
	self.getResult = function() {
		...
		jQuery.getJSON('/wp-content/plugins/smartpolis/php/ajax.php', jQuery('#smartpolis_car_form').serialize(), function(r) {
			...
			jQuery(r).each(function() {
				var company = this;
				jQuery.getJSON('/wp-content/plugins/smartpolis/php/ajax.php', { 'type':'getResult', 'id':company.id }, function(r){
					count_result--;
					jQuery('#smartpolis_wait_count_result').html('Осталось расчитать: ' + count_result);
...
jQuery(document).ready(function() {
	jQuery('#car_form').bind('submit', function() {
		jQuery.getJSON('/wp-content/plugins/smartpolis/php/ajax.php', jQuery(this).serialize(), function(r) {
			jQuery('#smartpolis_message_before_form').css('display', 'block');
